...
export function MediaPlayer({ media, isLoading }: MediaPlayerProps) {
  return (
    <div className="aspect-square w-full h-[300px] rounded-3xl bg-neutral-600 flex flex-row items-center justify-center">
      {media ? <MediaSearchPreview media={media} /> : null}
      {media == null && <>{isLoading ? <LoadingIcon /> : <PreviewIcon />}</>}
    </div>
  );
}
...
interface MediaSearchPreviewProps {
  media: MediaSearchResult;
}

function MediaSearchPreview({ media }: MediaSearchPreviewProps) {
  const url = useMemo(() => URL.createObjectURL(media.data), [media.data]);

  switch (media.type) {
    case "audio":
      return <audio className="w-full h-full rounded-3xl" controls src={url} />;
    case "video":
      return <video className="w-full h-full rounded-3xl" controls src={url} />;
    case "image":
      return (
